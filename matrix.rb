...
class Matrix
  ...
  # provide some structure that looks like a double array
  # Must implement fetch, each, and inspect
  def get_rows()
    @rows
  end
  ...
  def self.rows(rows, copy = true)
    ...
    size = (rows[0] || []).size
    rows.each do |row|
      self.Raise ErrDimensionMismatch, "row size differs (#{row.size} should be #{size})" unless row.size == size
    end
    new rows, size
  end
  ...
  def self.columns(columns)
    self.rows(columns, false).transpose
  end
  ...
  def self.scalar(n, value)
    self.diagonal(*Array.new(n, value))
  end
  ...
  def self.empty(row_size = 0, column_size = 0)
    self.Raise ArgumentError, "One size must be 0" if column_size != 0 && row_size != 0
    self.Raise ArgumentError, "Negative size" if column_size < 0 || row_size < 0
  ...
  #
  def [](i, j)
    self.get_rows().fetch(i){return nil}[j]
  end
  alias element []
  alias component []
  ...
  def row(i, &block) # :yield: e
    if block_given?
      self.get_rows().fetch(i){return self}.each(&block)
      self
    else
      Vector.elements(self.get_rows().fetch(i){return nil})
    end
  end
  ...
  def collect(&block) # :yield: e
    ...
    rows = self.get_rows().collect{|row| row.collect(&block)}
  ...
  def each(which = :all) # :yield: e
    ...
    when :diagonal
      self.get_rows().each_with_index do |row, row_index|
  ...
  def minor(*param)
    ...
    rows = self.get_rows()[from_row, size_row].collect{|row|
  ...
  def clone
    new_matrix self.get_rows().map(&:dup), column_size
  end
  ...
  def *(m) # m is matrix or vector or number
    case(m)
    when Numeric
      rows = self.get_rows().collect {|row|
        row.collect {|e| e * m }
      }
      return new_matrix rows, column_size
    when Vector
      m = self.column_vector(m)
      r = self * m
      return r.column(0)
    when Matrix
      self.Raise ErrDimensionMismatch if column_size != m.row_size
  ...
  def +(m)
    case m
    when Numeric
      self.Raise ErrOperationNotDefined, "+", self.class, m.class
    when Vector
      m = self.column_vector(m)
    when Matrix
    else
    ...
    self.Raise ErrDimensionMismatch unless row_size == m.row_size and column_size == m.column_size
  ...
  def ** (other)
    ...
    when Integer
      ...
      if other <= 0
        x = self.inverse
        return self.identity(self.column_size) if other == 0
        other = -other
      end
      z = nil
      loop do
  ...
  #
  def transpose
    return self.empty(column_size, 0) if row_size.zero?
    new_matrix self.get_rows().transpose, row_size
  end
  alias t transpose
  ...
  def to_s
    if empty?
      "#{self.class.name}.empty(#{row_size}, #{column_size})"
    else
      "#{self.class.name}[" + self.get_rows().collect{|row|
  ...
  def inspect
    if empty?
      ...
      "#{self.class.name}#{self.get_rows().inspect}"
    end
  end
